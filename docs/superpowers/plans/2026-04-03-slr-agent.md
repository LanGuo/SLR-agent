...
def create_search_subgraph(db: Database):
    """Routes based on search_sources config, calling PubMed and/or bioRxiv nodes and merging results."""
    builder = StateGraph(dict)
    ...
    builder.add_edge("fetch_abstracts", "search_biorxiv")
    builder.add_edge("search_biorxiv", END)
    return builder.compile()
```

Note: `create_search_subgraph` checks `config["search_sources"]` and calls `_search_biorxiv_node` if `"biorxiv"` is in sources. Results from both sources are merged before the deduplication/count step.

```python
def _search_biorxiv_node(state: dict, db: Database) -> dict:
    """Search bioRxiv using their internal search API via httpx."""
    import httpx, time
    pico = state["pico"]
    max_results = state.get("max_results", 500)
    run_id = state["run_id"]

    # bioRxiv search via their API endpoint
    query = " ".join([
        pico["population"], pico["intervention"], pico["comparator"], pico["outcome"]
    ])
    biorxiv_ids: set[str] = set()
    
    try:
        resp = httpx.get(
            "https://api.biorxiv.org/details/biorxiv/2020-01-01/2026-12-31/0/json",
            timeout=30,
        )
        # bioRxiv date-range API — returns preprints in date range
        # We fetch recent preprints and store them; Gemma 4 screens by PICO in stage 3
        data = resp.json()
        for item in data.get("collection", [])[:max_results]:
            ...
            if pmid_proxy in biorxiv_ids:
                continue
            biorxiv_ids.add(pmid_proxy)
            db.upsert_paper(PaperRecord(
                pmid=pmid_proxy,
                run_id=run_id,
                title=title,
    ...
    return {"_biorxiv_ids": list(biorxiv_ids)}
```

Note: bioRxiv's date-range API is used here since they have no keyword search endpoint. The Screening stage (stage 3) filters by PICO relevance using Gemma 4 — this is the appropriate place for semantic filtering.
